...
public class LojaItemMenu implements Listener
{
    ...
    public static void setarInventario(final Player p) {
        ...
        menu.setItem(2, criarItemMenu(Material.EMERALD_BLOCK, "§a§lLIGHT", new String[0]));
    ...
    public void abrirInv(final PlayerInteractEvent e) {
        ...
        if ((p.getItemInHand().getType() == Material.DIAMOND && API.getWarp(p) == "Spawn" && e.getAction() == Action.RIGHT_CLICK_AIR) || (e.getAction() == Action.RIGHT_CLICK_BLOCK && p.getItemInHand().hasItemMeta() && p.getItemInHand().getItemMeta().hasDisplayName() && p.getItemInHand().getItemMeta().getDisplayName().equals("§6» §7Loja"))) {
            setarInventario(p);
        }
    }
