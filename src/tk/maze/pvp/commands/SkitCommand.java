...
public class SkitCommand implements CommandExecutor {
	...
	public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
		if (!(sender instanceof Player)) {
			MazePvP.getInstance().getLogger().info("�7[�a!�7] �aComandos apenas no servidor!");
			return true;
		}
		final Player p = (Player) sender;
		if (cmd.getName().equalsIgnoreCase("skit")) {
			if (!p.hasPermission("HungerGames.c.Skit")) {
				p.sendMessage("�7[�a?�7] �7Voc\u00c3� n\u00c3�o possue permiss\u00c3�o para isso!");
				return true;
			}
			if (args.length <= 0) {
			...
			if (args[0].equalsIgnoreCase("create")) {
				...
				if (!MazePvP.skit.containsKey(args[1])) {
					MazePvP.skit.put(args[1], is);
					MazePvP.armor.put(args[1], armor);
				...
				if (args.length == 2) {
					if (MazePvP.skit.containsKey(args[1])) {
						...
						for (int length = (onlinePlayers = Bukkit.getOnlinePlayers()).length, i = 0; i < length; ++i) {
							...
							if (p != online && !AdminCommand.admin.contains(p)) {
								...
								online.getInventory().setContents((ItemStack[]) MazePvP.skit.get(args[1]));
								online.getInventory().setArmorContents((ItemStack[]) MazePvP.armor.get(args[1]));
								Bukkit.getLogger()
						...
						p.getInventory().setContents((ItemStack[]) MazePvP.skit.get(args[1]));
						p.getInventory().setArmorContents((ItemStack[]) MazePvP.armor.get(args[1]));
						return true;
					}
					...
					if (pl != null) {
						if (MazePvP.skit.containsKey(args[1])) {
							...
							pl.getInventory().setContents((ItemStack[]) MazePvP.skit.get(args[1]));
							pl.getInventory().setArmorContents((ItemStack[]) MazePvP.armor.get(args[1]));
							return true;
						}
				...
				if (MazePvP.skit.containsKey(args[1])) {
					MazePvP.skit.remove(args[1]);
					MazePvP.armor.remove(args[1]);
