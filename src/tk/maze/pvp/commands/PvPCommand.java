...
public class PvPCommand implements CommandExecutor, Listener
{
    ...
    public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
        ...
        if (cmd.getName().equalsIgnoreCase("pvp")) {
            if (!cfGrupo.ChecarGrupo(p, "Dono") && !cfGrupo.ChecarGrupo(p, "Gerente") && !cfGrupo.ChecarGrupo(p, "Gc") && !cfGrupo.ChecarGrupo(p, "Admin") && !cfGrupo.ChecarGrupo(p, "Mod+")) {
                API.sendMsg((CommandSender)p, "§b§lPVPGLOBAL: §7Voc\u00ea n\u00e3o possui permiss\u00e3o para utilizar este comando.");
                return true;
            }
            if (args.length == 0) {
