...
public class AdminCommand implements CommandExecutor, Listener
{
    ...
    private void interact(final PlayerInteractEvent e) {
        ...
        if ((p.getItemInHand().getType() == Material.SLIME_BALL && ArraysAPI.Admin.contains(p) && e.getAction() == Action.RIGHT_CLICK_AIR) || (e.getAction() == Action.RIGHT_CLICK_BLOCK && p.getItemInHand().hasItemMeta() && p.getItemInHand().getItemMeta().hasDisplayName() && p.getItemInHand().getItemMeta().getDisplayName().equals("§aTroca Rapida"))) {
            ...
            p.getInventory().setItem(4, API.criarItem(p, Material.SLIME_BALL, "§aTrocaRapida", new String[] { "" }, 1, (short)0));
    ...
    public boolean onCommand(final CommandSender Sender, final Command Cmd, final String Label, final String[] Args) {
        ...
        if (Cmd.getName().equalsIgnoreCase("admin")) {
            if (!cfGrupo.ChecarGrupo(p, "Dono") && !cfGrupo.ChecarGrupo(p, "Gerente") && !cfGrupo.ChecarGrupo(p, "Gc") && !cfGrupo.ChecarGrupo(p, "Admin") && !cfGrupo.ChecarGrupo(p, "Mod+") && !cfGrupo.ChecarGrupo(p, "Mod") && !cfGrupo.ChecarGrupo(p, "Trial") && !cfGrupo.ChecarGrupo(p, "Youtuber+") && !cfGrupo.ChecarGrupo(p, "Helper")) {
    ...
    public static void setAdmin(final Player p) {
        ...
        for (int length = (onlinePlayers = Bukkit.getOnlinePlayers()).length, i = 0; i < length; ++i) {
            final Player todos = onlinePlayers[i];
            if (cfGrupo.ChecarGrupo(todos, "Dono") || cfGrupo.ChecarGrupo(todos, "Gerente") || cfGrupo.ChecarGrupo(todos, "Gc") || cfGrupo.ChecarGrupo(todos, "Admin") || cfGrupo.ChecarGrupo(todos, "Gerente") || cfGrupo.ChecarGrupo(todos, "Mod+") || cfGrupo.ChecarGrupo(todos, "Mod")) {
                todos.showPlayer(p);
                p.showPlayer(todos);
            }
