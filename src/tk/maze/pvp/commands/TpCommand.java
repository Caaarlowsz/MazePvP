		...
		if (Cmd.getName().equalsIgnoreCase("tp")) {
			...
			if (Args.length > 2 || Args.length == 3) {
				if (!this.CheckarNumero(Args[0])) {
					p.sendMessage(
							"§b§lTP: §7\u00c9 necess\u00e1rio utilizar apenas n\u00fameros para teleportar para uma §c§lcoordenada.");
					return true;
				}
				...
				p.sendMessage("§b§lTP: §7Voc\u00ea foi §6§lTELEPORTADO §7at\u00e9 as coordenadas: §6§l" + X + " " + Y
						+ " " + Z);
