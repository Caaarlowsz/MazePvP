			...
			if (args.length == 1) {
				...
				if (sender instanceof Player) {
					...
					if (k != null) {
						...
						k.sendMessage(
								"§b§lGIVEYOUTUBER: §7O seu grupo(Youtuber) foi alterdo para Youtuber com sucesso!");
						TituloAPI.MandarTitulo(k, "§b§lYOUTUBER");
						TituloAPI.MandarSubTitulo(k,
								"§b§lGIVEYOUTUBER: §7O seu grupo foi alterado para §b§lYOUTUBER§7!");
				...
				if (!(sender instanceof Player)) {
					if (k == null) {
						final OfflinePlayer off = Bukkit.getOfflinePlayer(args[1]);
						sender.sendMessage(
								"§b§lGIVEYOUTUBER: §7Voc\u00ea aplicou o grupo Youtuber para" + off.getName());
						cfGrupo.setarGrupo(off, "Youtuber");
						cfGrupo.salvarconfiguracao();
						return true;
					}
