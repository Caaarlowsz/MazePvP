...
public class AddPermissaoCommand implements CommandExecutor {
	public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
		...
		if (cmd.getName().equalsIgnoreCase("addpermission")) {
			...
			p.sendMessage("§b§lPERMISSION: §7Permiss\u00e3o §6§l" + permissao + " §7adicionada ao jogador §6§l"
					+ k.getName());
		}
		if (cmd.getName().equalsIgnoreCase("removepermission")) {
			...
			p.sendMessage(
					"§b§lPERMISSION: §7Permiss\u00e3o §6§l" + permissao + " §7retirada do jogador §6§l" + k.getName());
		}
		return false;
	}
}
