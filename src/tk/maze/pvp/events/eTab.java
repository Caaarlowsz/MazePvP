    ...
    void TabDoServidor(final PlayerJoinEvent evento) {
        ...
        Bukkit.getScheduler().scheduleSyncRepeatingTask(Main.getPlugin(), (Runnable)new Runnable() {
            ...
            public void run() {
                ...
                final IChatBaseComponent top = ChatSerializer.a("{'extra': [{text: '', color: 'aqua'}],'color': gold, 'text': '        §b§lBROWK§f§lPVP             '}");
                final IChatBaseComponent bottom = ChatSerializer.a("{'extra': [{'color': 'aqua', 'text': ' \n      §7Kills: §b" + cfStatus.getKills(jogador) + "  §f-  §7Deaths: §b" + cfStatus.getDeaths(jogador) + "§f  -  §7KillStreak: §b" + cfStatus.getKillStreak(jogador) + "\n" + "§7Grupo: " + CheckUtils.Grupo(jogador) + "  §f-  §7Ping: §b" + ((CraftPlayer)jogador).getHandle().ping + "  §f-  §7XP: §b" + cfStatus.getMoney(jogador) + "\n" + "', 'underline': 'true'}], 'color': 'gold', 'text': ''}");
