...
public class UnbanCommand implements CommandExecutor {
	...
	public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
		...
		if (args.length == 0) {
			API.sendMsg((CommandSender) jogador,
					"§b§lUNBAN: §7Utilize /unban (Ban , Ipban , Tempban , Mute , Tempmute) (jogador)");
			return true;
		}
		if (args[0].equalsIgnoreCase("ban")) {
			if (args.length == 1) {
				API.sendMsg((CommandSender) jogador,
						"§b§lUNBAN: §7Utilize §6§l/unban §6(Ban , Ipban , Tempban , Mute , Tempmute) (jogador)");
				return true;
			}
			...
			if (!(sender instanceof Player)) {
				...
				API.sendMsg((CommandSender) jogador,
						"§b§lUNBAN: §7Voc\u00ea desbaniu o jogador: §6§l" + vitima.getName());
			...
			if (vitima2 == null) {
				...
				if (!(sender instanceof Player)) {
					...
					API.sendMsg((CommandSender) jogador,
							"§b§lUNBAN: §7Voc\u00ea desmutou o jogador: §6§l" + vitimaoff.getName());
					API.sendStaff(
							"§b§lUNBAN: §7" + jogador.getName() + " desmutou o jogador: §c§" + vitimaoff.getName());
				}
				...
				if (!(sender instanceof Player)) {
					...
					API.sendMsg((CommandSender) jogador,
							"§b§lUNBAN: §7Voc\u00ea desmutou o jogador: §c§l" + vitima2.getName());
					API.sendStaff(
							"§b§lUNBAN: §7" + jogador.getName() + " desmutou o jogador: §c§l" + vitima2.getName());
				}
			...
			if (!args[0].equalsIgnoreCase("tempmute")) {
				API.sendMsg(sender,
						"§b§lUNBAN: §7Utilize /unban §6(Ban , Ipban , Tempban , Mute , Tempmute) <jogador>");
				return true;
			}
			if (args.length == 1) {
			...
			if (vitima2 == null) {
				...
				if (!(sender instanceof Player)) {
					...
					API.sendMsg((CommandSender) jogador,
							"§b§lUNBAN: §7Voc\u00ea desmutou o jogador: §c§l" + vitimaoff.getName());
					API.sendStaff(
							"§b§lUNBAN: §7" + jogador.getName() + " desmutou o jogador: §c§l" + vitimaoff.getName());
				}
				...
				if (Config.getConfig().getTempMute().get("TempMute." + vitima2.getUniqueId()) == null) {
					API.sendMsg((CommandSender) jogador,
							"§b§lUNBAN: §7O jogador §6§l" + vitima2.getName() + "§f n\u00e3o est\u00e1 §4§lMUTADO§f!");
					return true;
				}
				if (!(sender instanceof Player)) {
