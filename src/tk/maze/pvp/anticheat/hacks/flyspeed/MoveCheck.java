					...
					synchronized (MoveCheck.Moves.get(Jogador)) {
						...
						for (final Jump jump : jumps) {
							...
							if (!jump.isOnGround && !Utills.Flight(Jogador)) {
								...
								if (jump.height > 5.0 || jump.isFloating) {
									Utills.Fly = Fly.getMenssagem().replace("nick", Jogador.getName())
											.replace("avisos",
													new StringBuilder().append(Utills.AvisosFly.get(Jogador))
															.toString())
							...
							if (!Utills.Speeding(Jogador)) {
								...
								if ((jump.horizontalSpeed > 10.0 && jump.time > 0.5)
										|| (jump.horizontalSpeed > 9.0 && jump.time > 1.5)) {
									Utills.Speed = Speed.getMenssagem().replace("nick", Jogador.getName())
											.replace("avisos",
													new StringBuilder().append(Utills.AvisosSpeed.get(Jogador))
															.toString())
											.replace("ping", new StringBuilder(String.valueOf(Ping)).toString());
	...
	public static ArrayList<Jump> GetJumps(final ArrayList<MoveLog> moves) {
		...
		while (inc < moves.size()) {
			...
			if (inc > startInc + 5) {
				...
				tmp79_76.getClass();
				final Jump jump = new Jump(moves.get(startInc), moves.get((inc + startInc - 1) / 2),
						moves.get(inc - 1));
			...
			while (inc < moves.size() && moves.get(inc).isAir
					&& moves.get(inc).location.getY() > moves.get(inc - 1).location.getY()) {
				++inc;
			}
			if (inc >= moves.size()) {
			...
			int floatCount = 0;
			while (inc < moves.size() && moves.get(inc).isAir) {
				if (moves.get(inc - 1).location.getY() == moves.get(inc).location.getY()
						&& !moves.get(inc).isOnLadder) {
					if (++floatCount > 3) {
						isFloating = true;
					}
				} else {
