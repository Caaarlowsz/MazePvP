...
public class Macro implements Listener
{
    ...
    private void onInventoryClickEvent(final InventoryClickEvent Evento) {
        ...
        if (Utills.MacroClicks.get(Jogador) >= 45) {
            Utills.Macro = MacroTalvez.getMenssagem().replace("nick", Jogador.getName()).replace("avisos", new StringBuilder().append(Utills.AvisosMacro.get(Jogador)).toString()).replace("clicks", new StringBuilder().append(Utills.ClicksMacro.get(Jogador)).toString());
        ...
        if (Utills.MacroClicks.get(Jogador) >= 55) {
            Utills.Macro = MacroProvavelmente.getMenssagem().replace("nick", Jogador.getName()).replace("avisos", new StringBuilder().append(Utills.AvisosMacro.get(Jogador)).toString()).replace("clicks", new StringBuilder().append(Utills.ClicksMacro.get(Jogador)).toString());
            Bukkit.getScheduler().scheduleSyncDelayedTask(Main.getPlugin(), (Runnable)new Runnable() {
                @Override
                public void run() {
                    if (Utills.Macro != null) {
        ...
        if (Utills.MacroClicks.get(Jogador) >= 65) {
            Utills.Macro = MacroDefinitivamente.getMenssagem().replace("nick", Jogador.getName()).replace("avisos", new StringBuilder().append(Utills.AvisosMacro.get(Jogador)).toString()).replace("clicks", new StringBuilder().append(Utills.ClicksMacro.get(Jogador)).toString());
            Bukkit.getScheduler().scheduleSyncDelayedTask(Main.getPlugin(), (Runnable)new Runnable() {
                @Override
                public void run() {
                    if (Utills.Macro != null) {
