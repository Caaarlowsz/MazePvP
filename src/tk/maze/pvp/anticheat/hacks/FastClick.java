...
public class FastClick implements Listener
{
    ...
    private void onPlayerInteractEvent(final PlayerInteractEvent Evento) {
        ...
        if (Utills.FastAttackClicks.get(Jogador) >= 50) {
            Utills.FastAttack = FastClickTalvez.getMenssagem().replace("nick", Jogador.getName().toString()).replace("avisos", new StringBuilder().append(Utills.AvisosFastClick.get(Jogador)).toString()).replace("clicks", new StringBuilder().append(Utills.ClicksFastClicks.get(Jogador)).toString());
        ...
        if (Utills.FastAttackClicks.get(Jogador) >= 60) {
            Utills.FastAttack = FastClickProvelmente.getMenssagem().replace("nick", Jogador.getName().toString()).replace("avisos", new StringBuilder().append(Utills.AvisosFastClick.get(Jogador)).toString()).replace("clicks", new StringBuilder().append(Utills.ClicksFastClicks.get(Jogador)).toString());
            Bukkit.getScheduler().scheduleSyncDelayedTask(Main.getPlugin(), (Runnable)new Runnable() {
                @Override
                public void run() {
                    if (Utills.FastAttackClicks.containsKey(Jogador)) {
        ...
        if (Utills.FastAttackClicks.get(Jogador) >= 80) {
            Utills.FastAttack = FastClickDefinitimante.getMenssagem().replace("nick", Jogador.getName().toString()).replace("avisos", new StringBuilder().append(Utills.AvisosFastClick.get(Jogador)).toString()).replace("clicks", new StringBuilder().append(Utills.ClicksFastClicks.get(Jogador)).toString());
            Bukkit.getScheduler().scheduleSyncDelayedTask(Main.getPlugin(), (Runnable)new Runnable() {
                @Override
                public void run() {
                    if (Utills.FastAttackClicks.containsKey(Jogador)) {
